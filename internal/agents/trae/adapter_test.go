...
func TestSettingsPathMultiplatform(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: "/custom/config/Trae/User/settings.json",
		},
		{
			name:    "Linux with default XDG_CONFIG_HOME",
			goos:    "linux",
			envVars: map[string]string{},
			want:    filepath.Join(home, ".config", "Trae", "User", "settings.json"),
		},
		{
			...
			want: "C:\\CustomAppData\\Trae\\User\\settings.json",
		},
		{
			name:    "Windows with default APPDATA",
			goos:    "windows",
			envVars: map[string]string{},
			want:    filepath.Join(home, "AppData", "Roaming", "Trae", "User", "settings.json"),
		},
		{
			name:    "macOS ignores environment variables",
			goos:    "darwin",
			envVars: map[string]string{},
			want:    filepath.Join(home, "Library", "Application Support", "Trae", "User", "settings.json"),
		},
	}
